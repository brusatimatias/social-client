#!/usr/bin/env bash
# social-client Stop hook: runs lint + typecheck + tests (same as CI)
# at the end of each turn, but only if the code changed since the last
# successful verification. If anything fails, it exits with code 2: Claude
# receives the error and keeps fixing.
...
# The whole repo except Claude config and documentation
...
# Attempt limit to avoid an infinite fix loop
...
if (( attempts > MAX_ATTEMPTS )); then
  rm -f "$ATTEMPTS_FILE"
  echo "verify.sh: reached the maximum of $MAX_ATTEMPTS attempts; check the results manually." >&2
  exit 0
fi
...
run_check "lint (oxlint)" npm run lint --silent
